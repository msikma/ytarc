...
const {fileExists, moveFile} = require('../../util')
...
const performArchivingTask = async (cmd, cwd) => {
  ...
  await moveFile(`${cwd}/${base}.mp4`, `${cwd}/${base}/${base}.mp4`)
...
const runArchiver = async (url, args, cwd) => {
  ...
  try {
    ...
    console.log(`[ytarc] Moving from "${tmpDir}" to "${cwd}/${archiveDir}"`)
    await moveFile(`${tmpDir}/${archiveDir}`, `${cwd}/${archiveDir}`)
